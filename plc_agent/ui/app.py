...
/* Button alignment: remove gray background container, align with input fields */
.analyze-btn {
...
def generate_llm_explanation_stream(program_key: str, error_code: str):
    """
    Stream LLM explanation chunk by chunk (generator).
    Yields accumulated text as it arrives from LLM.
    Direct LLM call, no Agent/tools — fast and stable.
    """
...
"""
    ...
    try:
        ...
        # Stream response and yield accumulated text
        accumulated = ""
        last_yield_len = 0
        ...
        for chunk in llm.stream(messages):
            if chunk.content:
                ...
                # Yield every ~80 chars or on newline for smooth typewriter effect
                if len(accumulated) - last_yield_len >= 80 or "\n" in chunk.content:
        ...
        yield f"文字分析生成失败: {type(e).__name__}: {str(e)}"
...
def run_analysis(region, line_no, equipment, error_code):
    ...
    GENERATOR: yields HTML updates progressively.
      1. First yield: summary + ladder diagrams + "正在分析..." placeholder
      2. Subsequent yields: summary + ladder diagrams + streaming LLM text
    """
    if not error_code or not error_code.strip():
        yield '<div class="placeholder-box"><span class="ph-icon">⚠️</span><span class="ph-text">请输入报警代码（如 F67）</span></div>'
        return
    ...
    if program_key not in PROGRAM_REGISTRY:
        yield f'<div class="placeholder-box"><span class="ph-icon">⚠️</span><span class="ph-text">产线 {program_key} 尚未接入系统</span></div>'
        return
    
    set_active_kb(program_key)
    
    # Phase 1: Generate SVG immediately
    ...
    # Build static part (summary + ladder diagrams)
    static_parts = [
        '<div class="output-area">',
        summary_html,
        '<h3>回溯梯形图</h3>',
    ]
    static_parts.extend(svg_htmls)
    static_html = "\n".join(static_parts)
    
    # First yield: show diagrams immediately + loading indicator
    loading_html = (
        static_html
        + '\n<h3>详细分析</h3>'
        + '\n<div style="margin:16px 0;color:#64748b;"><em>⏳ 正在生成分析...</em></div>'
        + '\n</div>'
    )
    yield loading_html
    
    # Phase 2: Stream LLM explanation with typewriter effect
    for accumulated_text in generate_llm_explanation_stream(program_key, device):
        explanation_html = _markdown_to_html(accumulated_text)
        full_html = (
            static_html
            + '\n<h3>详细分析</h3>'
            + f'\n<div style="margin:16px 0;">{explanation_html}</div>'
            + '\n</div>'
        )
        yield full_html
...
def build_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="PLC 智能诊断 Agent") as demo:
        ...
        # Input card — one-line search bar style (using Row, not Group)
        ...
        def on_analyze_click(region, line_no, equipment, error_code):
            if not error_code:
                yield '<div class="placeholder-box"><span class="ph-icon">⚠️</span><span class="ph-text">请输入报警代码</span></div>'
                return
            for html in run_analysis(region, line_no, equipment, error_code):
                yield html
